  ...
  const transaction = await prisma.$transaction(
    async (prismaTransaction) => {
      try {
        ...
        const total = await getDisabilityTypeTotal(
          prismaTransaction,
          searchText
        );
...
async function getDisabilitySubTypeByDisabilityTypeIdDBTransaction(
  id: string,
  sortOrder: sortOrderEnum = defaults.sortOrder
) {
  const transaction = await prisma.$transaction(
    async (prismaTransaction) => {
      try {
        ...
        const total = await getDisabilitySubTypeByDisabilityTypeIdDBTotal(
          prismaTransaction,
          id
        );
