...
async function disabilityCardFileWithDiffExtensionExists(
  personId: string,
  ...
  try {
    ...
    const fileNameWithoutExtension = generateDisabilityCardKeyWithoutExtension(
      personId,
      file,
      disabilityOfDivyangId
    );
...
async function fileWithDiffExtensionExists(
  personId: string,
  file: Express.Multer.File
) {
  try {
    log("info", "[fileExists]: Checking if file exists...");
    const fileNameWithoutExtension = generateKeyWithoutExtension(
      personId,
      file
    );
    const params: ListObjectsV2CommandInput = {
...
function generateKeyWithoutExtension(
  personId: string,
  file: Express.Multer.File
): string {
...
function generateDisabilityCardKeyWithoutExtension(
  personId: string,
  file: Express.Multer.File,
  disabilityOfDivyangId: string
): string {
  const key = path.join(
    personId,
    disabilityOfDivyangId,
    `DisabilityCard${path.extname(file.originalname)}`
  );
  log("info", "[generateDisabilityCardKeyWithoutExtension]: key: \n %s", key);
  return key;
}

function generateDisablityCardFileKey(
  personId: string,
  file: Express.Multer.File,
  disabilityOfDivyangId: string
): string {
...
async function saveDisabilityCardFileBufferToS3(
  personId: string,
  file: Express.Multer.File,
  disabilityOfDivyangId: string
): Promise<S3Result | undefined> {
  try {
    const oldFile = await disabilityCardFileWithDiffExtensionExists(
      personId,
      file,
      disabilityOfDivyangId
    );
    if (oldFile) {
      ...
      log(
        "info",
        "[saveDisabilityCardFileBufferToS3]: File already exists. Deleting it."
      );
      ...
      log(
        "info",
        "[saveDisabilityCardFileBufferToS3]: File Deleted. %o",
        s3Result
      );
    }
    ...
    log(
      "info",
      "[saveDisabilityCardFileBufferToS3]: File has been saved to S3. \n output: \n %o",
      output
    );
...
async function saveFileBufferToS3(
  personId: string,
  file: Express.Multer.File
): Promise<S3Result | undefined> {
  try {
    ...
    log(
      "info",
      "[saveFileBufferToS3]: File has been saved to S3. \n output: \n %o",
      output
    );
