...
const IdProofUploadsRequestSchema = z.object({
  ...
  panCardNumber: z.string().regex(/^[A-Z]{5}[0-9]{4}[A-Z]$/, "Invalid PAN card number format"),
  drivingLicense: z.string().length(64),
  rationCardNumber: z.string().length(64),
  aadharCardNumber: z.string().length(12).regex(/^\d{12}$/, "Invalid Aadhaar number format"),
